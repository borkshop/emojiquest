 ...
 * @prop {[width: number, height: number]} size
 * @prop {(x: number, y: number) => LayerCell|undefined} at
 */
...
export function curvedLayerParams(layer) {
  const { texture, cellSize, origin: [left, top], size: [width, height] } = layer;
  return {
    texture,
    cellSize,
    left: left - 0.5,
...
export function updateCurvedLayer(layer, tiles, isBaseCell) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      cell.tile = tiles.getLayerID(tileID);
    }
  }
}
...
export function clippedBaseCellQuery(layer, tiles) {
  const filled = tiles.getLayerID(0b1111);
  return (x, y) => layer.at(x, y)?.tile == filled ? 1 : 0;
}
