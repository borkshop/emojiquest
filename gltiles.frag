...
void main(void) {
  ...
  color = sheetLayer[0] < 0.0
    ? vec4(0)
    : texture(sheet, vec3(tileAt, sheetLayer[0]));
  if (anim_enabled && fragAnim.x > 0.0) {
    vec4 colorTo = sheetLayer[1] < 0.0
      ? vec4(0)
      : texture(sheet, vec3(tileAt, sheetLayer[1]));
    color = mix(color, colorTo, fragAnim.y);
  }
}
